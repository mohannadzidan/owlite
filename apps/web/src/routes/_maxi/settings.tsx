...
import { ChevronRightIcon, Keyboard, RotateCcw } from "lucide-react";
...
export const Route = createFileRoute("/_maxi/settings")({
...
function CaptureOverlay({ shortcut, onClose }: { shortcut: Shortcut; onClose: () => void }) {
  ...
  useEffect(() => {
    if (!lastCombo) return;
    if (lastCombo === staleComboRef.current) return;
